import asyncio
import logging
from aiogram import Bot, Dispatcher, types, F
from aiogram.filters import CommandStart, Command
from aiogram.types import InlineKeyboardMarkup, InlineKeyboardButton
from aiogram.fsm.storage.memory import MemoryStorage
import json
import os

# =====================
# SOZLAMALAR
# =====================
BOT_TOKEN = "8677339015:AAFJO3bngk6AVZqYGP2R88ldw5hVOCKwgho"  # @BotFather dan olingan token
CHANNEL_ID = "@englishhub1uz"  # Kanal username (@ bilan)
ADMIN_ID = 5027894185  # Sizning Telegram ID ingiz

# =====================
# DATA SAQLASH (JSON)
# =====================
DATA_FILE = "data.json"


def load_data():
    if os.path.exists(DATA_FILE):
        with open(DATA_FILE, "r") as f:
            return json.load(f)
    return {"users": {}, "referrals": {}}


def save_data(data):
    with open(DATA_FILE, "w") as f:
        json.dump(data, f, indent=2)


# =====================
# BOT VA DISPATCHER
# =====================
bot = Bot(token=BOT_TOKEN)
dp = Dispatcher(storage=MemoryStorage())


# =====================
# /start COMMAND
# =====================
@dp.message(CommandStart())
async def start(message: types.Message):
    data = load_data()
    user_id = str(message.from_user.id)
    username = message.from_user.username or message.from_user.full_name

    # Yangi foydalanuvchini ro'yxatdan o'tkazish
    if user_id not in data["users"]:
        data["users"][user_id] = {
            "username": username,
            "name": message.from_user.full_name,
            "invited": 0
        }

    # Referral tekshirish
    args = message.text.split()
    if len(args) > 1:
        referrer_id = args[1]
        if (referrer_id != user_id and
                referrer_id in data["users"] and
                user_id not in data.get("referrals", {})):

            # Referral hisoblash
            data["referrals"][user_id] = referrer_id
            data["users"][referrer_id]["invited"] += 1
            save_data(data)

            # Referrer ga xabar
            try:
                await bot.send_message(
                    int(referrer_id),
                    f"🎉 Tabriklaymiz! Yangi odam kanalingizga qo'shildi!\n"
                    f"👤 Kim: {message.from_user.full_name}\n"
                    f"📊 Jami taklif qilganlaringiz: {data['users'][referrer_id]['invited']} ta"
                )
            except:
                pass

    save_data(data)

    # Kanal a'zoligini tekshirish
    try:
        member = await bot.get_chat_member(CHANNEL_ID, message.from_user.id)
        is_member = member.status not in ["left", "kicked"]
    except:
        is_member = False

    # Foydalanuvchiga o'z referral linkini berish
    bot_info = await bot.get_me()
    ref_link = f"https://t.me/{bot_info.username}?start={user_id}"

    if is_member:
        await message.answer(
            f"👋 Salom, {message.from_user.full_name}!\n\n"
            f"🎯 Sizning referral linkingiz:\n{ref_link}\n\n"
            f"👥 Siz taklif qilganlar: {data['users'][user_id]['invited']} ta\n\n"
            f"🏆 Mukofotlar:\n"
            f"🥇 1-o'rin (eng ko'p) → 50 ⭐ Stars\n"
            f"🥈 2-o'rin → 25 ⭐ Stars\n\n"
            f"📢 Shu linkni do'stlaringizga yuboring!",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="📊 Mening natijam", callback_data="mystats")],
                [InlineKeyboardButton(text="🏆 Liderlar jadvali", callback_data="leaderboard")]
            ])
        )
    else:
        await message.answer(
            f"👋 Salom! Avval kanalga obuna bo'ling:",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="📢 Kanalga o'tish", url=f"https://t.me/{CHANNEL_ID.replace('@', '')}")],
                [InlineKeyboardButton(text="✅ Obuna bo'ldim", callback_data="check_sub")]
            ])
        )


# =====================
# OBUNA TEKSHIRISH
# =====================
@dp.callback_query(F.data == "check_sub")
async def check_subscription(callback: types.CallbackQuery):
    try:
        member = await bot.get_chat_member(CHANNEL_ID, callback.from_user.id)
        is_member = member.status not in ["left", "kicked"]
    except:
        is_member = False

    if is_member:
        data = load_data()
        user_id = str(callback.from_user.id)
        bot_info = await bot.get_me()
        ref_link = f"https://t.me/{bot_info.username}?start={user_id}"

        await callback.message.edit_text(
            f"✅ Rahmat! Kanalga obuna bo'ldingiz!\n\n"
            f"🔗 Sizning referral linkingiz:\n{ref_link}\n\n"
            f"👥 Taklif qilganlar: {data['users'].get(user_id, {}).get('invited', 0)} ta\n\n"
            f"🏆 Ko'proq odam taklif qiling va Stars yuting!",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="📊 Mening natijam", callback_data="mystats")],
                [InlineKeyboardButton(text="🏆 Liderlar jadvali", callback_data="leaderboard")]
            ])
        )
    else:
        await callback.answer("❌ Siz hali kanalga obuna bo'lmadingiz!", show_alert=True)


# =====================
# MENING NATIJAM
# =====================
@dp.callback_query(F.data == "mystats")
async def my_stats(callback: types.CallbackQuery):
    data = load_data()
    user_id = str(callback.from_user.id)
    user = data["users"].get(user_id, {})
    invited = user.get("invited", 0)

    # Reyting aniqlash
    sorted_users = sorted(
        data["users"].items(),
        key=lambda x: x[1].get("invited", 0),
        reverse=True
    )
    rank = next((i + 1 for i, (uid, _) in enumerate(sorted_users) if uid == user_id), "-")

    bot_info = await bot.get_me()
    ref_link = f"https://t.me/{bot_info.username}?start={user_id}"

    await callback.message.edit_text(
        f"📊 SIZNING NATIJANGIZ:\n\n"
        f"👤 Ism: {callback.from_user.full_name}\n"
        f"👥 Taklif qilganlar: {invited} ta\n"
        f"🏅 Reytingdagi o'rni: {rank}-o'rin\n\n"
        f"🔗 Linkingiz:\n{ref_link}",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="🏆 Liderlar jadvali", callback_data="leaderboard")],
            [InlineKeyboardButton(text="🔙 Orqaga", callback_data="back")]
        ])
    )
    await callback.answer()


# =====================
# LIDERLAR JADVALI
# =====================
@dp.callback_query(F.data == "leaderboard")
async def leaderboard(callback: types.CallbackQuery):
    data = load_data()

    sorted_users = sorted(
        data["users"].items(),
        key=lambda x: x[1].get("invited", 0),
        reverse=True
    )[:10]

    medals = ["🥇", "🥈", "🥉", "4️⃣", "5️⃣", "6️⃣", "7️⃣", "8️⃣", "9️⃣", "🔟"]
    text = "🏆 LIDERLAR JADVALI:\n\n"

    for i, (uid, udata) in enumerate(sorted_users):
        name = udata.get("name", "Noma'lum")
        invited = udata.get("invited", 0)
        medal = medals[i] if i < len(medals) else f"{i + 1}."
        text += f"{medal} {name} — {invited} ta\n"

    if not sorted_users:
        text += "Hali hech kim taklif qilmagan."

    await callback.message.edit_text(
        text,
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="🔙 Orqaga", callback_data="back")]
        ])
    )
    await callback.answer()


# =====================
# ORQAGA
# =====================
@dp.callback_query(F.data == "back")
async def go_back(callback: types.CallbackQuery):
    await start(callback.message)
    await callback.answer()


# =====================
# ADMIN — /stats COMMAND
# =====================
@dp.message(Command("stats"))
async def admin_stats(message: types.Message):
    if message.from_user.id != ADMIN_ID:
        return await message.answer("❌ Ruxsat yo'q!")

    data = load_data()
    sorted_users = sorted(
        data["users"].items(),
        key=lambda x: x[1].get("invited", 0),
        reverse=True
    )[:10]

    text = "📊 ADMIN — TO'LIQ STATISTIKA:\n\n"
    text += f"👥 Jami foydalanuvchilar: {len(data['users'])} ta\n\n"
    text += "🏆 TOP 10:\n"

    medals = ["🥇", "🥈", "🥉"]
    for i, (uid, udata) in enumerate(sorted_users):
        name = udata.get("name", "Noma'lum")
        username = udata.get("username", "-")
        invited = udata.get("invited", 0)
        medal = medals[i] if i < 3 else f"{i + 1}."
        text += f"{medal} {name} (@{username}) — {invited} ta\n"

    await message.answer(text)


# =====================
# ADMIN — /winner COMMAND
# =====================
@dp.message(Command("winner"))
async def announce_winner(message: types.Message):
    if message.from_user.id != ADMIN_ID:
        return await message.answer("❌ Ruxsat yo'q!")

    data = load_data()
    sorted_users = sorted(
        data["users"].items(),
        key=lambda x: x[1].get("invited", 0),
        reverse=True
    )

    if len(sorted_users) == 0:
        return await message.answer("Hali ishtirokchi yo'q!")

    text = "🎉 TANLOV YAKUNLANDI!\n\n🏆 G'OLIBLAR:\n\n"

    if len(sorted_users) >= 1:
        uid1, u1 = sorted_users[0]
        text += f"🥇 1-o'rin: {u1['name']} — {u1['invited']} ta taklif\n💰 Mukofot: 50 ⭐ Stars\n\n"

    if len(sorted_users) >= 2:
        uid2, u2 = sorted_users[1]
        text += f"🥈 2-o'rin: {u2['name']} — {u2['invited']} ta taklif\n💰 Mukofot: 25 ⭐ Stars\n\n"

    text += "Barcha ishtirokchilarga rahmat! 🙏"

    await message.answer(text)
    # Kanalga ham e'lon qilish
    try:
        await bot.send_message(CHANNEL_ID, text)
    except:
        pass


# =====================
# ISHGA TUSHIRISH
# =====================
async def main():
    logging.basicConfig(level=logging.INFO)
    print("✅ Bot ishga tushdi!")
    await dp.start_polling(bot)


if __name__ == "__main__":
    asyncio.run(main())